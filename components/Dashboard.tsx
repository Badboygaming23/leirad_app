...
const Dashboard: React.FC<DashboardProps> = ({ user, todos }) => {
  ...
  // 1. Tasks explicitly completed TODAY
  const completedTodayCount = todos.filter(t => 
    t.completed && t.completedAt && t.completedAt >= startOfToday && t.completedAt < endOfToday
  ).length;

  // 2. Tasks left to do: Overdue OR Due Today, and NOT completed
  const activeWorkloadCount = todos.filter(t => 
    !t.completed && t.dueDate && t.dueDate < endOfToday
  ).length;

  // 3. Total Daily Goal
  const totalDailyGoal = completedTodayCount + activeWorkloadCount;
  
  // 4. Calculate Progress
  const progress = totalDailyGoal === 0 ? 0 : Math.round((completedTodayCount / totalDailyGoal) * 100);
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div>
             ...
             <p className="text-indigo-100 font-medium text-sm leading-relaxed max-w-lg opacity-90">
               {overdueTasksCount > 0 
                 ...
                 : progress === 100 && totalDailyGoal > 0
                 ? "All targets hit for today. Incredible work!"
                 : "You're up to date. Ready to conquer the next task?"}
             </p>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 min-w-[240px] w-full md:w-auto shadow-sm">
             <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-bold text-indigo-100 uppercase tracking-wider">Today's Progress</span>
             ...
             <div className="grid grid-cols-2 gap-4 text-xs font-medium text-indigo-50">
                <div className="flex items-center gap-2">
                   ...
                   <span>{activeWorkloadCount} Left</span>
                </div>
                <div className="flex items-center gap-2">
